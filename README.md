...
| Public trend detection (`trend-detect`) | Reads Moltbook's `realtime`, `top`, and `discussed` feeds with the public GET API, applies a deterministic Marx-finance suitability gate, and returns candidates or `NO_ACTION`. It never generates comments, writes the outbox, or publishes. | Node 22 and dependencies. No Moltbook key, tracker, or Hermes required. |
...
`live` accepts `--article-url`, `--limit`, `--search-limit`, `--actions`,
`--output`, the optional `--with-agent-quote` (also accepted as
`--with-agent-quotes`), and the separate `--publish` opt-in. Quote mode adds a
complete, source-grounded Marx agent reply when one is available; it never
fabricates or clips a quote. It rejects unknown/duplicate options, fixture
overrides, disabled real models, and ambiguous boolean forms.
...
| Variable | Purpose |
| --- | --- |
| `MARX_GROWTH_CONFIG_DIR` | Directory containing `system.yaml`, `submolts.yaml`, `experiments.yaml`; also `feed.yaml` for feed commands. |
...
# Detect public realtime/top/discussed trend posts suitable for a Marx finance share.
# `--time day` mirrors the Moltbook UI's Today filter. This is always read-only.
node dist/cli/main.js trend-detect \
  --feeds realtime,top,discussed \
  --time day \
  --limit 20 \
  --min-score 0.58

# Rehearse the same detector without network access.
node dist/cli/main.js trend-detect \
...
For automatic new-feed detection, use `marx-feed-cycle`: it checks the public
Marx feed, persists unseen feed IDs in SQLite, and passes one pending feed URL
to `marx-specific-cycle`. It baselines existing feeds on first use and defaults
to drafts. See the [Hermes five-hour feed runbook](docs/hermes-feed-production.md)
for separate check/status commands, recovery, and `every 5h` scheduling.
...
For a reviewed top-level post, use the separate tracked POST handoff. It
replaces the exact canonical Marx URL in the supplied body with the tracker
URL, targets only the approved `general` submolt, publishes through the signed
Hermes contract, and finalizes the tracker only after the provider post ID and
permalink have been read back:

```bash
node scripts/publish-post.mjs \
  --title "Greenland deal: what actually changes beyond the security headline?" \
  --content-file /absolute/path/greenland-general-post.md \
  --destination-url https://marx.finance/feed/893d1114021c43c3b582575ad48d0953 \
  --submolt general
```

The command requires the same production tracker token, Hermes publisher
configuration, cleared kill switch, and exact receipt gates as comment
publishing. It leaves the tracker distribution pending when publication is
ambiguous; it never treats a missing receipt as success.
...
`trend-detect` is a separate read-side detector for the Moltbook UI feed modes.
It maps `realtime`, `top`, and `discussed` to the official `GET /api/v1/posts`
`sort` parameter and maps `--time day` to the UI's `time=day` filter. It scores
the root post for finance relevance, an agent bridge, specificity, trend signal,
thread saturation, and untrusted-content safety. Replies can affect safety and
saturation, but cannot turn an unrelated root post into a finance candidate.
The JSON envelope always reports `published: 0` and `outboxWrites: 0`.
...
A top-level POST action has no pre-existing source post identity:
...
```json
{
  ...
  "content": {
    "title": "Greenland deal: what actually changes beyond the security headline?",
    "content": "The final body with the tracked Marx URL.",
    "type": "text"
  }
}
```

The POST receipt binds `providerPostId` and the canonical
`https://www.moltbook.com/post/<id>` permalink. `targetPostId` remains reserved
for COMMENT actions and is never fabricated for a new post.
