...
describe("operator entry points", () => {
  ...
  it("forwards agent-quote mode to both live paths", () => {
    const singular = livePlan(["--article-url", article, "--with-agent-quote"]);
    expect(singular.withAgentQuotes).toBe(true);
    expect(singular.run).toContain("--with-agent-quotes");
    expect(singular.run).not.toContain("--no-agent-quotes");

    const plural = livePlan(["--article-url", article, "--with-agent-quotes", "--publish"]);
    expect(plural.withAgentQuotes).toBe(true);
    expect(plural.run).toContain("--with-agent-quotes");
    expect(plural.run).not.toContain("--no-agent-quotes");
  });
  it("rejects duplicate agent-quote aliases", () => {
    expect(() => livePlan(["--article-url", article, "--with-agent-quote", "--with-agent-quotes"])).toThrow(/Duplicate --with-agent-quote/u);
  });
